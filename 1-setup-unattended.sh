...
#######################################################################################################################
SERVER_NAME="Guacamole-VM4"                  # Server hostname (blank = use the current hostname)
LOCAL_DOMAIN="localdomain"                 # Local DNS namespace/domain suffix (blank = keep the current suffix)
...
MYSQL_ROOT_PWD="GuacamoleDBPa$$w0rd6734"               # Mandatory entry here or at script prompt
GUAC_PWD="GuacamolePa$$w0rd6734"                     # Mandatory entry here or at script prompt
...
INSTALL_NGINX="false"                # Install & configure Nginx reverse proxy http:80 frontend (true/false)
...
BACKUP_EMAIL="mysqlbackup22345@hotmail.com"                 # Email address to send MySQL backup notifications to
...
mv $USER_HOME_DIR/1-setup-unattended.sh $DOWNLOAD_DIR
